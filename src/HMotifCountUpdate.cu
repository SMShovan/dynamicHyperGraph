...
// Internal helper: run one anchored phase (subtract or add) into the given device buffer.
static void runAnchoredPhase(bool isAddition,
...
// MotifDeltaAccumulator implementation
void MotifDeltaAccumulator::init() {
    checkCudaLocal(cudaMalloc(&d_counts, 30 * sizeof(int)));
    checkCudaLocal(cudaMemset(d_counts, 0, 30 * sizeof(int)));
}

void MotifDeltaAccumulator::readTo(std::vector<int>& out) {
    out.resize(30);
    checkCudaLocal(cudaMemcpy(out.data(), d_counts, 30 * sizeof(int), cudaMemcpyDeviceToHost));
    checkCudaLocal(cudaFree(d_counts));
    d_counts = nullptr;
}

// Subtract phase: call BEFORE modifying data structures
void computeMotifSubtract(const CBSTContext& h2vCtx,
                          const CBSTContext& h2hCtx,
                          const std::vector<int>& deletedIds,
                          MotifDeltaAccumulator& acc) {
    runAnchoredPhase(false, h2vCtx, h2hCtx, deletedIds, acc.d_counts);
}

// Add phase: call AFTER building new snapshots
void computeMotifAdd(const CBSTContext& h2vCtx,
                     const CBSTContext& h2hCtx,
                     const std::vector<int>& insertedIds,
                     MotifDeltaAccumulator& acc) {
    runAnchoredPhase(true, h2vCtx, h2hCtx, insertedIds, acc.d_counts);
}

// Convenience wrapper (both contexts must be valid when called)
void computeMotifCountsDelta(const CBSTContext& oldH2vCtx,
    ...
    MotifDeltaAccumulator acc;
    acc.init();
    computeMotifSubtract(oldH2vCtx, oldH2hCtx, deletedHyperedgeIds, acc);
    computeMotifAdd(newH2vCtx, newH2hCtx, insertedHyperedgeIds, acc);
    acc.readTo(outDeltaCounts);
}
